...
void nb::Package::loadFromFolder(std::string path)
{
	...
	for (auto& el : filesystem::recursive_directory_iterator(p))
	{
		...
		else if (filesystem::is_regular_file(itpath))
	...
	{
		remove_if(listOfFilesInPackage.begin(), listOfFilesInPackage.end(), [&](const filesystem::path& vecEl) {
	...
	{
		MetaFile virtualMetaFile(el.string());
