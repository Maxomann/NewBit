...
void nb::ChunkSystem::onEntityAdded( Entity * entity )
{
	...
	if( transform )
	{
		...
		auto chunkPositon = calculateChunkPositionForPixelPosition( transform->getPosition().asPixelPosition() );
...
void nb::ChunkSystem::onEntitiesRemoved( const std::vector<Entity*>& entities )
{
	...
	for( const auto& el : entities )
	{
		...
		if( transform )
		{
			auto chunkPosition = calculateChunkPositionForPixelPosition( transform->getPosition().asPixelPosition() );
